...
# --- Gradio app logic ---
def process_user_message(user_text: str, chat_history: list):
    """Send message to agent and return updated chat history and image path."""
    ...
    try:
        ...
        out = aws_tool(user_text, prefer_plot=True)
    ...
    if isinstance(out, dict):
        if out.get("Action") == "metrics" and out.get("PlotPath"):
            image_path = out.get("PlotPath")
            # Convert to URL path for frontend
            plot_url = f"/plots/{os.path.basename(image_path)}"
            assistant_text = f"Plot generated. [View plot]({plot_url})"
        elif out.get("Action") == "list":
            assistant_text = f"Listed {out.get('Count', 0)} instances."
        elif out.get("Status") == "Error":
            assistant_text = out.get("Message", "Error occurred.")
        else:
            assistant_text = "Operation completed."
    else:
...
# --- Build Gradio UI ---
with gr.Blocks(title="AWS Agent (Gradio)") as demo:
    gr.Markdown("# AWS Assistant — Gradio UI\nType commands like `list instances in mumbai` or `show cpu for i-0123456789abcdef in the last 6 hours`")

    chatbot = gr.Chatbot(label="AWS Assistant Chat", type="messages")
    last_image = gr.Image(type="filepath", label="Last plot (if any)")

    with gr.Row():
        txt = gr.Textbox(placeholder="Type your command...", show_label=False)
        send = gr.Button("Send")

    def submit_fn(user_text, history):
        h, img = process_user_message(user_text, history)
        return h, img

    send.click(submit_fn, inputs=[txt, chatbot], outputs=[chatbot, last_image])
    txt.submit(submit_fn, inputs=[txt, chatbot], outputs=[chatbot, last_image])

    gr.Markdown("---\n**Notes:** This UI runs the same Agent as your CLI version. Ensure AWS creds are configured and the strands/Bedrock pieces are available in your environment.")


if __name__ == '__main__':
    demo.launch()
    # Mount the plots directory for static file serving after launch
    demo.app.mount("/plots", StaticFiles(directory=PLOT_DIR), name="plots")
